...
<div class='network' nid='' id="div_templateNetwork">
    <div class="container-fluid">
        <div id="content">
                <div id="config_scenarios" class="tab-pane">
			...
			$modules =knocklet::getModuleList();
                        foreach(scenario::all() as $scio)
                        {
                               echo  '<tr ID="'.$scio->getId().'"><td><b>'.$scio->getName().'</b></td>';
 			       echo '<td><select class="sel_bracelet form-control" data-l1key="object_id"><option value="">{{Aucun}}</option>';
                               foreach ($bracelets as $object)
                               {
                                        if(knocklet::getTripletFromScioId($scio->getId())["braceletId"]==$object["id"])
                                        {
			       ...
			       if(knocklet::getTripletFromScioId($scio->getId())["braceletId"]=="*")
                                {
				...
				{
					if(knocklet::getTripletFromScioId($scio->getId())["moduleId"]==$object["id"])
					{
						echo '<option value="' . $object["id"] . '" selected>' . $object["name"] . '</option>';	
					}
					else echo '<option value="' . $object["id"] . '">' . $object["name"] . '</option>';
				...
				if(knocklet::getTripletFromScioId($scio->getId())["moduleId"]=="*")
                                {
                                        echo '<option value="*" selected> Tous </option>';
                                }else echo '<option value="*"> Tous </option>';
			       ...
			       echo '<td><select class="sel_knock form-control"><option value="">{{Aucun}}</option>';
                               ...
                               for ($x=2;$x<=6;$x++)
                                {
					if(knocklet::getTripletFromScioId($scio->getId())["knocks"]==$x)
                                        {
                ...
                ?>
            	    ...
            	    <div class="btn btn-success eqLogicAction" id="saveScios"><i class="fa fa-check-circle"></i>{{Sauvegarder}}</div>
