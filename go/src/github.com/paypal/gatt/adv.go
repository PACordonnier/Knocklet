...
// FIXME: check the nmarshalling of this data structure.
...
func (a *Advertisement) unmarshall(b []byte) error {
	...
	uuidListServiceData := func(s []ServiceData, d []byte, w int) []ServiceData {
		for len(d) > 0 {
			sd := ServiceData{
				UUID: UUID{d[:w]},
				Data: d[w:],
	...
	for len(b) > 0 {
		...
		case typeServiceData128:
			a.ServiceData = uuidListServiceData(a.ServiceData, d, 16)
		default:
			log.Printf("DATA: [ % X ]", d)
		}
		b = b[1+l:]
